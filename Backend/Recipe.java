...
public class Recipe {
     ...
     * 3. Appends the filename "Recipes.bin" to the path and checks if the file exists. If not, it calls makeDefaultList(1).
     ...
     * 6. Throws a ClassCastException if the loaded file does not contain a valid ArrayList of recipes, then exits the JVM if any exception was thrown.
     ...
     */
    public static ArrayList<Recipe> loadSavedList() throws IOException, IllegalArgumentException {
        ...
        if (!f.exists()) {
            ...
            throw new IOException(System.getProperty("user.dir") + "\\.RecipeBrowser\\Recipes.bin" + " is a folder, where it should be a file. Execution cannot continue.");
        }

        // Load the file and return the list of recipes
        ArrayList<Recipe> recipesList = new ArrayList<Recipe>(); // avoid compiler warnings about uninitialized value within Try block which cannot actually be uninitialized
        try (ObjectInputStream objIn = new ObjectInputStream(new FileInputStream(importPath))) {
            ...
            if (obj instanceof ArrayList<?>) {
                ...
                throw new ClassCastException("Recipes.bin is not an ArrayList of Recipes");
            }
        } catch (Exception e) {
            // Handle exceptions and exit on failure
            System.out.println("Exception thrown reading serialized object: " + e.getMessage());
            System.out.println("Execution cannot continue without a valid default recipes list. Exiting.");
            System.exit(-1);
        }
    ...
    public static void writeDefaultList(int state) throws IOException, IllegalArgumentException{
        ...
        if (state == 1) {
            ...
            ObjectOutputStream objOut = new ObjectOutputStream(new FileOutputStream(System.getProperty("user.dir") + "\\.RecipeBrowser\\Recipes.bin"));
    ...
    /**
     * Generates and returns a default list of recipes. This method does not make any changes to the filesystem.
     * The generated list includes several common recipes with the removable flag set to false. 
     * Any new recipes needed for new recipes added to the backend should be added here.
     *
     * @return An ArrayList of Ingredient objects representing the default recipes list.
     */
    public static ArrayList<Recipe> makeDefaultList(){
        ...
        Recipe one = new Recipe("Salmon with Brown Sugar Glaze",
    ...
    /**
     * Constructs a new Recipe object with the specified name, instructions, preparation time, and serving count.
     *
     ...
     * @param isDefault Whether the recipe is one of the defaults, used to signal the frontend not to allow deletion of it.
     */
    public Recipe(String nameIn, String instructionsIn, String prepTimeIn, int servingCountIn, boolean isDefault) {
        name = nameIn;
     ...
     * @throws IllegalArgumentException if a number not between 1 and 5 (inclusive) is entered.
     */
    public void addRating(int input) throws IllegalArgumentException {
        if (input < 1 || input > 5) throw new IllegalArgumentException("Ratings must be from 1-5 (inclusive) but a " + input + " was provided");
        else ratings.add(input);
    }

    /**
     * Calculates and returns the average rating of the recipe based on user ratings.
     *
     ...
     */
    public RecipeIngredient getIngredient(int index) throws IllegalArgumentException{
        if (index >= this.getIngredientCount()) throw new IllegalArgumentException("The Index provided is higher than the list of ingredients allows.");
     ...
     * @param quantity The quantity or amount of the ingredient to be added to the recipe.
     */
     ...
     */
    public void addIngredient(String ingredient, String quantity, String substitutions) {
    ...
    /**
     * Removes an ingredient from the recipe by name. It will remove all duplicate names, if they exist.
     * 
