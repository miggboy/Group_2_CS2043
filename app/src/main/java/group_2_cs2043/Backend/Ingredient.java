...
/**
 * The `Ingredient` class represents an ingredient in a recipe. It provides
 * methods for managing the ingredient's properties,
 * such as name, availability, and removability. Additionally, the class offers
 * functionality to load and create default
...
public class Ingredient implements Serializable {
  ...
  /**
   * Loads the saved list of ingredients from disk or calls the makeDefaultList
   * method with a specific state if the file is not present.
   ...
   * error.
   * 3. Appends the filename "Ingredients.bin" to the path and checks if the file
   ...
   * ArrayList of Ingredients, then exits the JVM if any exception was thrown.
   ...
   * - If 'state' is 1, it generates a default ingredients list, serializes it,
  ...
  /**
   * Generates and returns a default list of ingredients. This method does not
   * make any changes to the filesystem.
   * The generated list includes several common ingredients with the removable
   * flag set to false.
   * Any new ingredients needed for new recipies added to the backend should be
   * added here.
   *
   * @return An ArrayList of Ingredient objects representing the default
   *         ingredients list.
   */
  ...
  /**
   * Constructs a new Ingredient object with the specified name and removable
   * status.
   *
   ...
   *
   * @param in A boolean value indicating whether the ingredient is available or
   *           not.
   */
  ...
  /**
   * Sets the removable status of the ingredient. It should be set if it is used
   * in any recipie, and unset if the last recipie using it is removed.
   *
   * @param isRemovable A boolean value indicating whether the ingredient is
   *                    removable or not.
   */
